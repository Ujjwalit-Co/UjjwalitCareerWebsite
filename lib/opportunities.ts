...
export async function getPreviousPrograms(): Promise<Opportunity[]> {
  if (!hasSupabaseConfig()) return [];

  try {
    // Use the admin client so closed/archived programs are readable even if
    // the relaxed public-read RLS policy hasn't been applied to the DB yet.
    ...
    console.error('Failed to load previous programs.', error);
    return [];
  }
}
...
export async function getOpportunityBySlug(slug: string): Promise<Opportunity | null> {
  ...
  try {
    // Admin client so closed/archived program detail pages work too, and
    // regardless of the RLS migration state.
    const supabase = hasServiceRole() ? createAdminClient() : await createClient();
    const { data, error } = await supabase
      .from('opportunities')
      .select('*')
