...
import ShareBadgeCard from '@/components/verify/ShareBadgeCard';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { formatDate, getCertificateTypeLabel } from '@/lib/utils';
import {
  ArrowLeft, Award, BadgeCheck, BookOpen, ChevronDown, Download, ExternalLink,
  GraduationCap, Loader2, Mail, MessageSquare, SearchX, ShieldCheck, Sparkles, Share2,
} from 'lucide-react';
...
export default function StudentProfilePage({ slug }: { slug: string }) {
  const [isLoading, setIsLoading] = useState(true);
  const [notFound, setNotFound] = useState(false);
  const [data, setData] = useState<ProfileData | null>(null);
  const [openStatementId, setOpenStatementId] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'overview' | 'certificates' | 'badge'>('overview');
  ...
  const hasStatements = (st: StudentEntry): boolean =>
    (st.achievement_statements || []).length > 0;
  ...
  return (
    <div className="w-full max-w-2xl space-y-5 px-4 pb-16 sm:px-0">
      ...
      {/* Tabs controller */}
      <div className="flex items-center gap-1.5 bg-stone-950 p-1 border border-stone-800 rounded-xl max-w-md w-full">
        <button
          onClick={() => setActiveTab('overview')}
          className={`flex-1 py-2 px-3 rounded-lg text-xs font-semibold text-center transition-all ${
            activeTab === 'overview' ? 'bg-blue-500/15 text-blue-400 border border-blue-500/10 font-bold' : 'text-stone-500 hover:text-stone-200'
          }`}
        >
          <BookOpen size={12} className="inline mr-1" /> Enrollments ({students.length})
        </button>
        <button
          onClick={() => setActiveTab('certificates')}
          className={`flex-1 py-2 px-3 rounded-lg text-xs font-semibold text-center transition-all ${
            activeTab === 'certificates' ? 'bg-blue-500/15 text-blue-400 border border-blue-500/10 font-bold' : 'text-stone-500 hover:text-stone-200'
          }`}
        >
          <Award size={12} className="inline mr-1" /> Certificates ({certificates.length})
        </button>
        <button
          onClick={() => setActiveTab('badge')}
          className={`flex-1 py-2 px-3 rounded-lg text-xs font-semibold text-center transition-all ${
            activeTab === 'badge' ? 'bg-blue-500/15 text-blue-400 border border-blue-500/10 font-bold' : 'text-stone-500 hover:text-stone-200'
          }`}
        >
          <Share2 size={12} className="inline mr-1" /> Share Badge
        </button>
      </div>

      {/* ENROLLMENTS */}
      {activeTab === 'overview' && students.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {students.map((st) => {
              const hasStatement = hasStatements(st);
              const certStyle = CERT_STYLE[st.certificate_type] ?? CERT_STYLE.participation;
              const open = openStatementId === st.id;
              return (
                <div
                  key={st.id}
                  className="group rounded-2xl border border-stone-800 bg-stone-950 overflow-hidden transition-colors hover:border-stone-700"
                  onMouseEnter={() => setOpenStatementId(st.id)}
                  onMouseLeave={() => setOpenStatementId((cur) => (cur === st.id ? null : cur))}
                >
                  <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                      ...
                      {hasStatement && (
                        <button
                          onClick={(e) => { e.stopPropagation(); setOpenStatementId(open ? null : st.id); }}
                          className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[10px] font-bold transition-colors cursor-pointer ${
                            open
                              ? 'border-amber-500/40 bg-amber-500/10 text-amber-400'
                              : 'border-stone-700 bg-stone-900 text-stone-400 hover:text-amber-400 hover:border-amber-500/40'
                          }`}
                        >
                          <Sparkles size={10} className={open ? 'text-amber-400' : 'text-amber-500/70'} />
                  ...
                  {/* Accordion body — reveals on hover, stays pinned on click */}
                  <div
                    className={`grid transition-all duration-300 ease-in-out ${
                      open ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0 group-hover:grid-rows-[1fr] group-hover:opacity-100'
                    }`}
                  >
                    <div className="overflow-hidden">
                      {hasStatement && (
                        <div className="border-t border-stone-800/70 bg-stone-900/40 px-4 py-3">
                          <div className="space-y-3">
                            {(st.achievement_statements || []).map((s) => {
                              const body = s.body_markdown
                                .replaceAll('{{attendance}}', String(st.attendance_percentage ?? 0))
                                .replaceAll('{{batch}}', st.batch_name || '');
                              return (
                                <div key={s.id} className="rounded-lg border border-stone-800/80 bg-stone-950/60 px-3.5 py-3">
                                  <p className="mb-1.5 text-[11px] font-extrabold uppercase tracking-wide text-teal-400">
                                    {s.label}
                                  </p>
                                  <div className="prose prose-invert prose-sm max-w-none text-stone-300 prose-p:my-0 prose-p:leading-6 prose-strong:text-amber-300 prose-strong:font-extrabold prose-ul:my-0 prose-li:my-0.5 prose-li:leading-6">
      ...
      {/* SHARE BADGE */}
      {activeTab === 'badge' && (
        <section className="relative overflow-hidden rounded-2xl border border-stone-800 bg-stone-950 p-5 sm:p-6">
          <div className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-blue-600/10 blur-3xl" />
          <div className="relative">
            <h2 className="mb-1 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-stone-600">
              <span className="h-px flex-1 bg-stone-800" />Share Your Badge<span className="h-px flex-1 bg-stone-800" />
            </h2>
            <p className="mb-4 text-sm text-stone-400 leading-relaxed">
              Download a verified Ujjwalit Developers Program badge and share it on LinkedIn, your portfolio, or anywhere online.
            </p>
            <ShareBadgeCard
              options={students.map((st) => {
                const cert = certificates.find((c) => c.student.student_code === st.student_code) ?? null;
                return {
                  key: st.id,
                  fullName: profile.full_name,
                  college: st.application?.college ?? '',
                  programTitle: st.opportunity?.title ?? '',
                  batchName: st.batch_name ?? '',
                  studentCode: st.student_code ?? '',
                  certificateId: cert?.certificate_id ?? null,
                  certificateType: cert?.certificate_type ?? null,
                  issueDate: cert?.issued_at ?? null,
                  label: st.opportunity?.title ?? st.student_code,
                  sublabel: st.batch_name ?? '',
                };
              })}
              profileUrl={`https://verify.ujjwalit.co.in/student/${profile.slug}`}
            />
          </div>
        </section>
      )}
