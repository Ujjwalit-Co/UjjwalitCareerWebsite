...
import { PreviousProgramsSection } from '@/components/careers/PreviousProgramsSection';
import { getOpenOpportunities, getPreviousPrograms } from '@/lib/opportunities';

export const dynamic = 'force-dynamic';

export default async function CareersLandingPage() {
  const [opportunities, previousPrograms] = await Promise.all([getOpenOpportunities(), getPreviousPrograms()]);
  ...
  return (
    <div className="w-full flex flex-col bg-brand-bg text-[#F5F5F5]">
      ...
      <PreviousProgramsSection programs={previousPrograms} />
      <TransparencySection />
    </div>
  );
}
